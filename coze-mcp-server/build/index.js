...
class CozeAPIClient {
    ...
    async getChatStatus(chatId, conversationId) {
        const response = await fetch(`https://api.coze.cn/v3/chat/retrieve?chat_id=${chatId}&conversation_id=${conversationId}`, { headers: this.getHeaders() });
        return await response.json();
    }
    /**
     * 获取消息列表
    ...
    async getMessageList(chatId, conversationId) {
        const response = await fetch(`https://api.coze.cn/v3/chat/message/list?chat_id=${chatId}&conversation_id=${conversationId}`, { headers: this.getHeaders() });
        return await response.json();
    }
}
/**
 ...
 * @returns Promise<string> - 返回内容字符串
 */
async function getCozeContent(keyword) {
    try {
